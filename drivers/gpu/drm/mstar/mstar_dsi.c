...
/*
 * MStar DSI driver. Probably the same hardware as the MediaTek one.
 *
 * Copytight (c) 2021 Daniel Palmer <daniel@thingy.jp>
...
static const struct of_device_id mstar_dsi_of_match[] = {
	{
		.compatible = "sstar,ssd20xd-dsi",
		.data = &ssd20xd_dsi_driver_data
	},
	{}
};
MODULE_DEVICE_TABLE(of, mstar_dsi_of_match);

struct platform_driver mstar_dsi_driver = {
	.probe = mstar_dsi_probe,
	.remove = mstar_dsi_remove,
	.driver = {
		.name = DRIVER_NAME,
		.of_match_table = mstar_dsi_of_match,
	},
};
module_platform_driver(mstar_dsi_driver);
