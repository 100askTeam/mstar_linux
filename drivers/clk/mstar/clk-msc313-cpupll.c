...
{
	riu_writel_relaxed_abs(cpupll->base + REG_LPF_HIGH_BOTTOM, regvalue);

	writew_relaxed(0x1, cpupll->base + REG_LPF_MYSTERYONE);
	writew_relaxed(0x6, cpupll->base + REG_LPF_MYSTERYTWO);
	writew_relaxed(0x8, cpupll->base + REG_LPF_UPDATE_COUNT);
	writew_relaxed(BIT(12), cpupll->base + REG_LPF_TRANSITIONCTRL);

	writew_relaxed(0, cpupll->base + REG_LPF_TOGGLE);
	writew_relaxed(1, cpupll->base + REG_LPF_TOGGLE);

	while (!(readw_relaxed(cpupll->base + REG_LPF_LOCK)))
		cpu_relax();

	writew_relaxed(0, cpupll->base + REG_LPF_TOGGLE);

	riu_writel_relaxed_abs(cpupll->base + REG_LPF_LOW_L, regvalue);
}
...
static unsigned long msc313_cpupll_recalc_rate(struct clk_hw *hw, unsigned long parent_rate)
{
	...
	return msc313_cpupll_frequencyforreg(riu_readl_relaxed_abs(cpupll->base + REG_LPF_LOW_L), parent_rate);
}
